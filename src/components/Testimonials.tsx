...
export const Testimonials = () => {
  ...
  const change = (updater: (prev: number) => number) => {
    setVisible(false);
    window.setTimeout(() => {
      setI(updater);
      setVisible(true);
  ...
  const next = () => change((p) => (p + 1) % items.length);
  const prev = () => change((p) => (p - 1 + items.length) % items.length);
  const goTo = (idx: number) => change(() => idx);
  const cur = items[i];
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-center gap-4 mt-6">
              ...
              <div className="flex gap-2">
                {items.map((_, idx) => (
                  <button key={idx} onClick={() => goTo(idx)} aria-label={`${t("testimonials.review")} ${idx + 1}`} className={`h-2 rounded-full transition-[width,background-color] duration-300 ease-out ${idx === i ? "w-8 bg-gradient-gold" : "w-2 bg-border"}`} />
                ))}
              </div>
